...
Essa é uma configuração chamada _On-screeen Display_. Ela sobrepõe sobre o vídeo um texto. Nesse caso, é possível **renomear** o nome do canal de acordo com o local que a câmera está instalada.
...
Essa configuração habilita a detecção de movimento. Selecione o canal que deseja configurar e marque a opção **_Habilitar Detecção de Movimento_**. Os parâmetros configuráveis são:
...
| Gravar ao ter movimento         | Quando há detecção de movimento, é feita a gravação de acordo com os canais selecionados. Para isso, clique em _Configurações_ para definir os canais.                                                             |
| Tirar snapshot ao ter movimento | Quando há detecção de movimento, é tirada uma foto de acordo com os canais selecionados. Para isso, clique em _Configurações_ para definir os canais.                                                              |
...
Atualiza o _firmware_ do HVR a partir de um dispositivo USB.
